...
static void handle_scroll_or_arrow_input(struct pixart_data *data, const struct device *dev,
    ...
    if (input_mode == SCROLL) {
        ...
        if (abs(x) > CONFIG_PMW3610_SCROLL_TICK) {
            uint32_t keycode = x > 0 ? HID_USAGE_KEY_KEYBOARD_RIGHTARROW : HID_USAGE_KEY_KEYBOARD_LEFTARROW;
        ...
        if (abs(y) > CONFIG_PMW3610_SCROLL_TICK) {
            uint32_t keycode = y > 0 ? HID_USAGE_KEY_KEYBOARD_DOWNARROW : HID_USAGE_KEY_KEYBOARD_UPARROW;
            zmk_hid_press(keycode);
            k_msleep(10);
            zmk_hid_release(keycode);
        }
